...
function impoverished_multinomial_recombination(data::AbstractMatrix{T}, data_description, max_exponent::Int) where T
    """
    You might be tempted to apply a list of transforms arbitrarily, but 
    then you might end up with equivalent transforms, so that the columns
    are not independent.
    ...
    const_idx = 1
    base_idx = 2
    first_derivative_idx = 3
    @assert string(data_description[const_idx]) == "C"
    @assert string(data_description[base_idx]) == "u"
    
    len_vars, n_vars = size(data)
    n_derivatives = n_vars - first_derivative_idx + 1
    n_terms = n_vars + n_derivatives * max_exponent + (max_exponent - 1)
    multinomial_data = Matrix{T}(undef, len_vars, n_terms)
